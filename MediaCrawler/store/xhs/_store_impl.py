...
class XhsMongoStoreImplement(AbstractStore):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            _utils.logger.info("[XhsMongoStoreImplement] Initialized Mongo store with prefix 'xhs'")
        except Exception:
            pass
    ...
    async def store_content(self, content_item: Dict):
        ...
        # 保存到 MongoDB 的集合：使用前缀 + 'notes'，例如 'xhs_notes'
        result = await self.mongo_store.save_or_update(
            collection_suffix="notes",
        ...
        if result:
            ...
            utils.logger.error(f"[XhsMongoStoreImplement.store_content] Failed to save note {note_id} to MongoDB")
    ...
    async def store_comment(self, comment_item: Dict):
        ...
        if result:
            ...
            utils.logger.error(f"[XhsMongoStoreImplement.store_comment] Failed to save comment {comment_id} to MongoDB")
    ...
    async def store_creator(self, creator_item: Dict):
        ...
        # 保存到 MongoDB 的集合：使用前缀 + 'users'，例如 'xhs_users'
        result = await self.mongo_store.save_or_update(
        ...
        if result:
            ...
            utils.logger.error(f"[XhsMongoStoreImplement.store_creator] Failed to save creator {user_id} to MongoDB")
