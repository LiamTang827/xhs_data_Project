...
class XhsStoreFactory:
    ...
    def create_store() -> AbstractStore:
        ...
        utils.logger.info(f"[XhsStoreFactory] Creating store for option '{config.SAVE_DATA_OPTION}': {store_class.__name__}")
        return store_class()
...
# Note: do NOT re-export the legacy file-based implementations from `xhs_store.py` here,
# because they will override the factory-based implementations above and force
# local JSON storage. The factory (`XhsStoreFactory.create_store()`) will select
# the correct backend (including MongoDB) based on `config.SAVE_DATA_OPTION`.

# If you need the legacy functions for direct file access, import them explicitly:
# from .xhs_store import save_creator as legacy_save_creator
