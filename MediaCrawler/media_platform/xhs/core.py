    ...
    async def get_creators_and_notes(self) -> None:
        ...
        for creator_url in config.XHS_CREATOR_ID_LIST:
            try:
                ...
                # if only get creator info, skip get notes
                if config.XHS_ONLY_GET_CREATOR_INFO:
                    utils.logger.info(f"[XiaoHongShuCrawler.get_creators_and_notes] Only get creator info, skip get notes, creator_url:{creator_url}")
                    continue
            ...
            try:
                ...
                utils.logger.error(f"[XiaoHongShuCrawler.get_creators_and_notes] Exception when calling get_all_notes_by_creator for {user_id}: {e}")
                utils.logger.error(traceback.format_exc())
                # skip this creator on exception
                continue

            # Defensive checks: ensure iterable and contains dicts with note_id
            if not all_notes_list:
                utils.logger.warning(f"[XiaoHongShuCrawler.get_creators_and_notes] No notes returned for creator {user_id}. Response: {repr(all_notes_list)[:500]}")
                continue

            if not isinstance(all_notes_list, (list, tuple)):
                utils.logger.error(f"[XiaoHongShuCrawler.get_creators_and_notes] Unexpected response type from get_all_notes_by_creator for {user_id}: {type(all_notes_list)}. Response snippet: {repr(all_notes_list)[:500]}")
                continue
            ...
            for note_item in all_notes_list:
                try:
                    if not note_item or not isinstance(note_item, dict):
                        utils.logger.debug(f"[XiaoHongShuCrawler.get_creators_and_notes] Skipping invalid note_item for {user_id}: {repr(note_item)[:200]}")
                        continue
                    nid = note_item.get("note_id")
                    if not nid:
                        utils.logger.debug(f"[XiaoHongShuCrawler.get_creators_and_notes] note_item missing note_id for {user_id}: {repr(note_item)[:200]}")
                        continue
                    note_ids.append(nid)
                    xsec_tokens.append(note_item.get("xsec_token", ""))
                except Exception:
                    utils.logger.error(f"[XiaoHongShuCrawler.get_creators_and_notes] Error processing note_item for {user_id}: {repr(note_item)[:200]}")
                    utils.logger.error(traceback.format_exc())
            ...
            if note_ids:
                try:
                    ...
                    utils.logger.error(f"[XiaoHongShuCrawler.get_creators_and_notes] Exception in batch_get_note_comments for {user_id}: {e}")
                    utils.logger.error(traceback.format_exc())
            ...
            try:
                if note_detail:
                    ...
                    try:
                        ...
                        utils.logger.error(f"[fetch_creator_notes_detail] get_notice_media failed for note: {note_detail.get('note_id')}")
                        utils.logger.error(traceback.format_exc())
            except Exception:
                utils.logger.error(f"[fetch_creator_notes_detail] Error handling note_detail: {repr(note_detail)[:500]}")
                utils.logger.error(traceback.format_exc())
