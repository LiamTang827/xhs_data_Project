...
class XiaoHongShuClient(AbstractApiClient):
    ...
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        ...
        # Try to decode JSON; if fails, include raw response snippet in the error
        try:
            data: Dict = response.json()
        except Exception as e:
            raw = response.text
            utils.logger.error(f"[XiaoHongShuClient.request] Failed to parse JSON response for {url}: {e}. Response snippet: {repr(raw)[:1000]}")
            raise DataFetchError(f"Invalid JSON response: {repr(raw)[:1000]}")
        ...
        try:
            ...
            # defensive: if data isn't a mapping as expected
            utils.logger.error(f"[XiaoHongShuClient.request] Unexpected JSON structure for {url}: {repr(data)[:1000]}")
            raise DataFetchError(f"Unexpected JSON structure: {repr(data)[:1000]}")

        # Handle known error codes
        if data.get("code") == self.IP_ERROR_CODE:
            raise IPBlockError(self.IP_ERROR_STR)

        # Otherwise, include available message or the response snippet to aid debugging
        msg = data.get("msg") if isinstance(data, dict) else None
        detail = msg or repr(data)[:1000]
    ...
    async def get_all_notes_by_creator(
        ...
        while notes_has_more and len(result) < config.CRAWLER_MAX_NOTES_COUNT:
            try:
                ...
                utils.logger.error(f"[XiaoHongShuClient.get_all_notes_by_creator] API error fetching notes for {user_id}: {e}")
                # Try HTML fallback once when API fails or returns unexpected JSON
                try:
                    html_notes = await self.get_notes_by_creator_from_html(user_id)
                    if html_notes:
                        utils.logger.info(f"[XiaoHongShuClient.get_all_notes_by_creator] Fallback extracted {len(html_notes)} notes from HTML for user {user_id}")
                        # treat these as final results (no pagination from HTML fallback)
                        if callback:
                            await callback(html_notes)
                        result.extend(html_notes)
                        break
                    else:
                        utils.logger.warning(f"[XiaoHongShuClient.get_all_notes_by_creator] HTML fallback returned no notes for {user_id}")
                        break
                except Exception:
                    utils.logger.error(f"[XiaoHongShuClient.get_all_notes_by_creator] HTML fallback also failed for {user_id}")
                    utils.logger.error(traceback.format_exc())
                    break
            # Diagnostic: log a short snippet of the raw response to help debug when notes are missing
            try:
                utils.logger.info(f"[XiaoHongShuClient.get_all_notes_by_creator] raw notes_res snippet: {repr(notes_res)[:500]}")
            except Exception:
                pass
            if not notes_res:
                utils.logger.error(
    ...
    async def get_notes_by_creator_from_html(self, user_id: str) -> List[Dict]:
        """
        Fallback: fetch the user's profile HTML and attempt to extract note ids from
        window.__INITIAL_STATE__ embedded JSON. Returns a list of dicts with
        at least the key 'note_id'. This method is best-effort and defensive.
        """
        try:
            ...
            utils.logger.error(f"[XiaoHongShuClient.get_notes_by_creator_from_html] Failed to fetch profile HTML for {user_id}: {e}")
            return []

        # Attempt to extract initial state JSON
        import re, json
        ...
        try:
            ...
            utils.logger.error(f"[XiaoHongShuClient.get_notes_by_creator_from_html] Failed to parse INITIAL_STATE JSON for {user_id}")
            return []

        # Search recursively for lists that look like posts (dicts containing id-like keys)
        def find_post_list(obj):
            results = []
            if isinstance(obj, dict):
                for v in obj.values():
                    results.extend(find_post_list(v))
            elif isinstance(obj, list):
                # heuristic: list of dicts with id-like keys
                if len(obj) > 0 and isinstance(obj[0], dict):
                    first = obj[0]
                    for key in ("note_id", "id", "noteId", "post_id", "postId"):
                        if key in first:
                            for item in obj:
                                nid = item.get("note_id") or item.get("id") or item.get("noteId") or item.get("post_id") or item.get("postId")
                                if nid:
                                    results.append({"note_id": str(nid), "xsec_token": "", "xsec_source": ""})
                            return results
                # otherwise, descend into elements
                for elem in obj:
                    results.extend(find_post_list(elem))
            return results

        posts = find_post_list(state)
        return posts
