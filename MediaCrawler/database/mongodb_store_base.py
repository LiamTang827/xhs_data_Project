...
class MongoDBConnection:
    ...
    async def _connect(self):
        """建立连接"""
        try:
            # 优先使用环境变量 MONGO_URI（便于在 .env 中统一配置）
            ...
            if mongo_uri:
                ...
                # 回退到基于 host/port/user/password 的配置
            ...
            # Log connection target safely. If MONGO_URI was used, try to extract host info, otherwise use host/port
            try:
                if mongo_uri:
                    from urllib.parse import urlparse

                    parsed = urlparse(mongo_uri)
                    host_log = parsed.hostname or "<uri>"
                    port_log = parsed.port or ""
                else:
                    host_log = host
                    port_log = port
            except Exception:
            ...
            utils.logger.info(f"[MongoDBConnection] Connected to {host_log}:{port_log}/{db_name}")
